...
MailShield の位置付け・必要な外部コンポーネント・システム要件を説明する。**セットアップを始める前に必ず読むこと。**

| 項目 | 内容 |
|------|------|
| 対象読者 | MailShield の導入を検討・計画している管理者 |
...
> [!NOTE]
> EML はすべてアーカイブされるため、ディスク使用量はメール流量に比例して増加する。容量計画は平均メールサイズ × 日次流量 × 保持期間で見積もること。
...
> [!WARNING]
> 受信 MTA の `content_filter` 設定と同じポートに再インジェクトすると、
...
> [!WARNING]
> 通知 SMTP も受信 MTA の `content_filter` ポートに送ると**ループ**します。
