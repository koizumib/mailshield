...
Docker を使わず、ソースからビルドしたバイナリを systemd で常駐させる手順。インフラ（MariaDB・MinIO・RabbitMQ・Redis）は別途用意されていることを前提とする。
...
| RabbitMQ | 3.13 以上 | オプション（`queue.backend: none` で省略可） |
...
### 3.3 RabbitMQ の Exchange 作成（queue.backend: rabbitmq の場合）

```bash
# definitions.json をインポートする（mail.received Exchange が定義されている）
rabbitmqadmin import docker/infra/rabbitmq/definitions.json
```

> [!IMPORTANT]
> smtp-gateway は Exchange を passive declare（存在確認のみ）するため、`mail.received` Exchange が存在しないと起動に失敗する。

## 4. 設定

`/opt/mailshield/config/mailshield.yaml` を編集する。全パラメータは [設定リファレンス](../specs/configuration.md) を参照。

```yaml
server:
  smtp_port: 10024
  trusted_sources:
    - <postfix-hostname-or-ip>    # 受信 MTA からの接続を許可する

storage:
  backend: filesystem             # minio / s3 / filesystem
  local_dir: /opt/mailshield/data # filesystem の場合
  # backend: minio の場合:
...
queue:
  backend: none                   # rabbitmq / none

reinject:
  host: <mta-host>                # 処理済みメールを戻す MTA（content_filter なしポート）
  port: 10025
```

ルート定義（受信・送信ドメイン）とポリシーは `config/routes.d/` 配下で設定する。[MailShield 設定ガイド](./mailshield-config.md) の Step 3〜4 を参照。

パスワード類は環境変数ファイルにまとめる。

```bash
# /opt/mailshield/.env
DB_PASSWORD=<db-password>
MINIO_ACCESS_KEY=<access-key>      # storage.backend: minio / s3 の場合
MINIO_SECRET_KEY=<secret-key>
RABBITMQ_USER=mailshield           # queue.backend: rabbitmq の場合
RABBITMQ_PASSWORD=<rabbitmq-password>
MAILSHIELD_CONFIG_DIR=/opt/mailshield/config
CONFIG_FILE=/opt/mailshield/config/api-server.yaml
```

```bash
sudo chown mailshield:mailshield /opt/mailshield/.env
sudo chmod 600 /opt/mailshield/.env
```

## 5. 動作確認（フォアグラウンド起動）

systemd に登録する前に、一度フォアグラウンドで起動して設定を検証する。

```bash
cd /opt/mailshield
sudo -u mailshield env $(grep -v '^#' .env | xargs) ./bin/smtp-gateway
```

期待されるログ（JSON・抜粋）:

```json
{"level":"INFO","msg":"MariaDB 接続完了","host":"<mariadb-host>"}
{"level":"INFO","msg":"ルート初期化完了","route":"inbound", ...}
{"level":"INFO","msg":"smtp-gateway 起動完了","smtp_port":10024,"health_port":8080,"routes":3}
```
