...
**変更履歴とロールバック:** UI の「履歴」から過去の版に復元できる。編集・復元のたびに
変更前の内容が保存されるため、誤った変更を元に戻せる。復元操作も監査ログ（`policy.rolled_back`）に残る。

**シナリオテンプレート:** 「テンプレート」から定型ルールセット（外部メールのタグ付け・
フリーメール宛送信の承認・業務時間外ディレイ・実行ファイル添付の除去など）をフォールバックの
手前に挿入できる。挿入後に条件やアクションを調整して保存する。
