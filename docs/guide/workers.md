...
各ワーカーの詳細設定は `config/workers/{name}.yaml` に記述します。
...
**設定（`config/workers/sanitize-worker.yaml`）:**
```yaml
...
**設定（`config/workers/filesep-worker/config.yaml`）:**
```yaml
mode: inline          # inline（本文冒頭にリンク挿入）| separate（リンクのみのメールを送信）

inline_template:    /app/config/workers/filesep-worker/inline-template.txt
separate_template:  /app/config/workers/filesep-worker/separate-template.txt
