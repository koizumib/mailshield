...
ワーカーはルートごとに `mailshield.yaml` で有効化します。

```yaml
routes:
  - name: inbound
    workers:
      workers_dir: /app/workers          # Lua ワーカーのディレクトリ
      worker_config_dir: /app/config/workers/conf  # ワーカー設定ファイルのディレクトリ
      inspect:                           # 検査ワーカー（並列実行）
        - name: av-worker
          enabled: true
          timeout_seconds: 30
...
workers:
  ...
  inspect:
    - name: my-custom-worker    # workers/my-custom-worker/init.lua
      enabled: true
      timeout_seconds: 5
```
