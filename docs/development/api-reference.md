...
- **メールボックス承認**（主・`mailbox_emails` が非空）: 対象メールボックス（1..n）のいずれかに role=admin で割り当てられたユーザー全員が決定できる（先に決定した人が有効）。受信メールで宛先が複数の場合、admin がいるすべての宛先メールボックスが対象になる
...
> [!NOTE]
> 承認者はメールボックスの `role=admin` 割り当てで決まる。メールボックス管理 API
