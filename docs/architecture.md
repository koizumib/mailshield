...
受信と送信は**同一バイナリ（smtp-gateway）**が処理する。`config/routes.d/` 配下のルートディレクトリで正規表現によるルート振り分けを定義し、ルートごとにワーカーとポリシーを切り替える。
...
受信・送信の両方を担う単一サービス。`config/routes.d/` のルートディレクトリを起動時に読み込んで動作を切り替える。
...
3. MariaDB にメタデータ記録
4. `mail.received` イベント発行（webhook・オプション）
5. 検査パイプライン（並列）
6. 変換パイプライン（直列）
7. ポリシー評価・アクション実行
...
- `approval`: 承認フロー保留（承認者への通知・期限切れ処理あり）
...
│   ├── mailshield.yaml              # smtp-gateway 設定（trusted_sources 等）
│   ├── api-server.yaml              # api-server 設定
│   ├── routes.d/                    # ルート定義（ディレクトリ名順で評価）
│   │   ├── 00-bounce/               # バウンスルート
│   │   │   ├── route.yaml
│   │   │   └── policy.yaml
│   │   ├── 10-inbound/              # 受信ルート
...
│   │       ├── policy/              # ポリシーエンジン
│   │       ├── events/              # mail.received イベント発行（webhook / noop）
│   │       ├── repository/          # DB アダプター
