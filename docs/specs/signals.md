...
6. HTTPヘルスチェックサーバーを停止する（タイムアウト: **5秒** 固定）
7. MariaDB・RabbitMQ の接続は `defer` で自動クローズされる
...
`docker compose -f docker/docker-compose.yml stop` のデフォルトタイムアウトは 10 秒であるため、`--timeout 35` 程度を指定することを推奨する。
