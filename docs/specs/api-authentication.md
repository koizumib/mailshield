...
**ロールごとに解決方式（method）を独立して選べる:**

| method | データの流れ | 向いている環境 |
|---|---|---|
| `user_attribute` | ユーザー自身の属性（`memberOf` 等）→ 変換 → 再検索（任意・最大1回）→ 属性読み取り → 変換 | 汎用。SCIM の `User.groups` にも将来同じ考え方を適用できる |
...
例:「member/owner はユーザーの `memberOf` から、admin は固定リスト」のような混在ができる。
