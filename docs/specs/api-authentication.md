...
リクエスト受信
    │
    ▼
Cookie に mailshield_session が存在するか？
    │ Yes → Redis でセッション検索 → 有効なら通過
    │ No  ↓
    ▼
Authorization: Bearer <key> ヘッダが存在するか？
    │ Yes → SHA-256(key) でDBを検索 → IsActive() なら通過・last_used_at を非同期更新
    │ No  ↓
    ▼
401 UNAUTHORIZED
```
...
POST /api/v1/auth/login
  { "email": "...", "password": "..." }
    → 200 OK + Set-Cookie: mailshield_session=<session_id>
```

```
GET /api/v1/auth/login/oidc   # OIDC プロバイダーへリダイレクト
GET /api/v1/auth/callback     # OIDC コールバック → セッション発行
```
