...
5. `mailbox_mappings` が設定されている場合、同じユーザーの `attributes.groups` からメールボックス割り当ても同期する（詳細は次項）

設定項目の詳細は [設定リファレンスの directory.ldap](configuration.md#directoryldap) を参照。

### メールボックス割り当ての自動反映（LDAP mailbox_mappings）

`mailboxes` / `mailbox_assignments`（隔離メールの可視性を viewer ロールに対して制限する仕組み。詳細は [ストレージ仕様](storage.md) や `docs/architecture.md` を参照）も、LDAP のグループ所属から自動反映できる。

**発見方法は「ユーザー起点」のみをサポートする。** メールボックス一覧を LDAP 側から自己記述的に発見する方式（グループ自身の `mail`/`member`/`managedBy` 属性を読む方式）は実装していない。理由は、SCIM の Group リソース（RFC 7643）がメールアドレス属性を持たないため、将来 SCIM 連携を追加したときに同じ仕組みを再利用できないため。ユーザー起点（`memberOf` → グループ → mailbox+role）の解決方法だけが LDAP・SCIM の両方でポータブルである。

**グループ → メールボックス + role の解決方法は 2 通りあり、併用できる:**

| 方法 | 向いている組織 | 設定コスト |
|---|---|---|
| 明示リスト（`mailbox_mappings.list`） | メールボックス数が少ない | メールボックスごとに手書き |
| 命名規則（`mailbox_mappings.pattern`） | メールボックス数が多い・グループ命名規則を統一できる | 正規表現 1 つを最初に書くだけ。以後グループを増やしても設定変更不要 |

**メールボックスの自動作成について:** `mailbox_mappings` が指すメールボックスが存在しない場合、自動的に作成する（`provisioned_by = ldap`）。これは LDAP のディレクトリデータ（`mail` 属性等）を信用しているのではなく、**`mailbox_mappings` の設定自体が管理者によって明示的に書かれた値**だからである（Web UI の「メールボックス作成」フォームに入力するのと同じ信頼レベル）。

**権威モデルは `users.provisioned_by` と同じ:** `manual`（Web UI 手動作成・追加）の `mailboxes` 行・`mailbox_assignments` 行は、LDAP 同期・JIT のどちらからも一切変更されない。

**反映タイミングは 2 箇所:**
- **LDAP bind ログイン時（JIT）**: ログインした本人 1 人分の割り当てのみを対象に作成・削除する。他ユーザーへの影響が無いスコープなので、ログイン直後に本人のメールボックスが即座に見えるようにするために安全に実行できる
- **定期同期（`Syncer`）**: 全ユーザー分をまとめて反映する。ログインしていないユーザーの離脱（グループから外れた→アクセス剥奪）もここで拾う
