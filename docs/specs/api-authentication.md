...
**メールボックスは「すべての有効なメールアドレス」分が必要になる。** viewer ロールのユーザーが自分宛の隔離メールを閲覧・解放できるのは `mailbox_assignments` 経由の権限だけなので、共有メールボックス（ML・部署アドレス）に加えて**各ユーザーの個人メールアドレス自体もメールボックスとして登録し、本人を割り当てる**必要がある。個人メールボックスは `chain: [{self: mail}, {to_mailbox: {}}]`（自分のメールアドレス属性をそのままメールボックスにする・再検索不要）と書くだけで自動反映できる。

**設定はルールのリスト（`rules`）で、各ルールは `chain` / `lua` / `fixed` のいずれか 1 つを持つ。同じ role に複数のルールを書ける（結果は合算される）:**

| 解決方法 | データの流れ | 向いている環境 |
|---|---|---|
| `chain` | ユーザーエントリを起点に線形ステップ（`self`/`const` → `regex`/`search`/`attr` → `to_mailbox`）を上から適用 | 汎用。`self: mail` なら個人メールボックス、`self: memberOf` なら共有メールボックス、`search` でグループ再検索。SCIM の `User.groups` にも将来同じ考え方を適用できる |
| `lua` | `resolve(user)` を実装した Lua スクリプトが `{mailbox, role}` のリストを返す | chain で表現できない変則ディレクトリ。ユーザー属性の変換のみ（LDAP 再検索は不可） |
| `fixed` | 設定に列挙したユーザー → ldap 管理下の全メールボックス | 全メールボックスを見る管理者・全体承認者の決め打ち |

例:「個人メールボックスは本人が owner、共有メールボックスは `memberOf` から member、全体承認者は固定リスト」のように、同一 role を含む複数ルールの混在ができる。

**`chain` は線形パイプラインとして設計されている（分岐・ループ・無制限の多段参照は許さない）:**
- 最初のステップは必ず `self`（ユーザー属性起点）か `const`（固定値起点）、最後は必ず `to_mailbox`（アドレス確定）でなければならない
- `search`（再検索）の `filter` に埋め込む `{value}` は実装側で必ず LDAP エスケープする（インジェクション対策・無効化不可）。同一クエリは同期サイクル内で `(base_dn, filter)` キャッシュされ N+1 を防ぐ
- `regex` の「マッチしない値はスキップ」という挙動が、`memberOf` に混在する無関係なグループの除外フィルタを兼ねる
- `lua` フックは `resolve(user)` の中でユーザー属性（`user.attrs.<name>`・`user.dn`）を参照して変換するのみで、LDAP 再検索の手段は渡さない（検索ロジックは chain に寄せ、Lua は変則的な文字列整形に限定する）
...
**反映タイミングは 2 箇所（どちらもユーザー単位でチェーンを解決する）:**
- **LDAP bind ログイン時（JIT）**: ログインした本人 1 人分だけを反映する。チェーンの `search` ステップはそのユーザーの値ごとに 1 クエリ（キャッシュ付き）で解決する。解決用の LDAP 接続・検索に失敗した場合はそのユーザーの反映自体を行わない（不完全なタプル集合での reconcile は正当な割り当ての誤削除につながるため、現状維持を選ぶ）
- **定期同期（`Syncer`）**: 全ユーザー分を 1 人ずつ反映する。`fixed` 対象ユーザーは第2パス（他ユーザーの反映でメールボックスが出揃った後）に処理する。あるユーザーのチェーン解決（`search` 等）が失敗した場合、そのユーザーの反映だけをスキップして次のユーザーへ進む（他ユーザーの反映とユーザー同期は続行）
