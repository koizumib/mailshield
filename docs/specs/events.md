# 統合イベント仕様（mail.received webhook）

smtp-gateway は `mail.received` イベントを **webhook（HTTP POST）** で外部システムへ通知できる。
...
- ヘッダー:
  ...
  - `X-MailShield-Event: mail.received`
...
- 発行タイミングはメール受信直後（EML 保存・DB 記録完了後）。
  リトライを含む発行全体は `server.event_publish_timeout_seconds`（デフォルト 5 秒）で打ち切られる
