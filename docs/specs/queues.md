...
smtp-gateway は `mail.received` イベントの**発行のみ**行う。ワーカーは smtp-gateway 内でインプロセス実行されるため、ワーカー間のキュー通信は存在しない。
...
`mail.received` Exchange は smtp-gateway が起動する前に RabbitMQ 上に存在していなければならない。smtp-gateway は起動時に passive declare（存在確認のみ）を行い、存在しない場合は起動に失敗する。Exchange の作成は `docker/infra/rabbitmq/definitions.json` で行う。
...
| `none` | 発行をスキップする（noop モード）。RabbitMQ 不要 |

`none` モードは RabbitMQ が不要な単一ノード構成・開発環境向け。発行はスキップされるが、処理は正常に続行する。
...
外部プロセス・サービスとして検査ワーカーを実装する場合は、以下のキュー構成が想定される。smtp-gateway が発行するのは `mail.received` のみであり、残りのキューは外部システムが担う。
