...
**変換パイプライン失敗時の動作:** 実配送経路（[6/7]）と同じセマンティクスで報告する。変換ワーカーが 1 つでも失敗した場合、ポリシー評価は行わず `action: "quarantine"` と `transform_error`（失敗理由）を返す。実配送では変換失敗メールは未処理のまま配送されず隔離されるため、`/simulate` も「変換なしで配送される」という実態と異なる結果を返さない。
